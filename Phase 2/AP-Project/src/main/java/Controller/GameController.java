...
//            FileController.addKingdomToFile(newKingdom);
    ...
    public String dropObject(Matcher matcher) {
        ...
        switch (matcher.group("object")) {
            case "unit" -> result = dropUnit(cell, type);
    ...
    public String dropUnit(Cell cell, String type) {
        ArrayList<Person> kingUnitsCopy = new ArrayList<>(Objects.requireNonNull(currentGame.getKingdomByKing
                (currentGame.turn.getCurrentKing().getUsername())).getKingPeople());
...
//        return "success";
    ...
    public void nextTurn() {
        ...
        if (currentGame.turn.getCurrentKing().getUsername().equals(currentGame.getKingdoms().get(1).getKing().getUsername())) {
            fight();
            arrowsAct();
            hittingBuildings();
            checkToHitBuilding();
    ...
    public static String buyFromShop2(Product product, int amount) {
        if (product.getCost() * amount > currentGame.getKingdomByKing(currentGame.turn.getCurrentKing().getUsername()).getInventory())
            ...
            currentGame.getKingdomByKing(currentGame.turn.getCurrentKing().getUsername()).setInventory(currentGame.getKingdomByKing(currentGame.turn.getCurrentKing().getUsername()).getInventory()
                    - product.getCost() * amount);
            double count = 0;
            if (currentGame.getKingdomByKing(currentGame.turn.getCurrentKing().getUsername()).getKingProductByName(product.getName()) == null) {
                Product product1 = FileController.getProductByName(product.getName());
                currentGame.getKingdomByKing(currentGame.turn.getCurrentKing().getUsername()).addKingProduct(product1);
            }
